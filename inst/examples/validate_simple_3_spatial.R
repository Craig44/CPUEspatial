...
#' non-linear spatial variable Compare method with normal GAM approach
#'
library(TMB)
library(CPUEspatial)
library(INLA)
...
continuous_spline_var_model = RMwhittle(nu = nu, var = depth_sigma2, scale = depth_range * 1.5 )
...
proj_df$spline_var <- RFsimulate(depth_model, x=as.matrix(proj_df[,c("x","y")]), exactness=TRUE)$variable1 + 6

ggplot(proj_df, aes(x = x, y = y, fill = depth)) + 
  geom_tile()

ggplot(proj_df, aes(x = x, y = y, fill = spline_var)) + 
  geom_tile()
...
spline_var = seq(min(proj_df$spline_var), max(proj_df$spline_var), by = 0.1)
plot(spline_var, sin(spline_var), xlab = "Spline Var", ylab = "effect on Y", type = "l", lwd  =3)
...
sampData$spline_var = proj_df$spline_var[ndx]

cor(sampData$depth, sampData$spline_var)
...
sampData$eta =  intercept + normalised_year_coef[sampData$year] + normalised_region_coef[sampData$region] + ifelse(sampData$fleet_ndx == 1, 0, fleet_coef[2] - fleet_coef[1]) + sin(sampData$spline_var) 
...
gam_fit_spa = gam(y_i ~ factor(year) + factor(fleet_ndx) + factor(region) + s(spline_var, bs = 'cs'), offset = log(area), data = sampData, family = Gamma(link = log), control = list(maxit = 10000), method = "REML")

## Compare the fits
opt_spa$objective 
logLik(gam_fit_spa)
## get year indes
con_ndx_spa_glm = canonical.index(GLM = gam_fit_spa, year = 1990:1999, base  = 1, year.name = "year")
...
1 / summary(gam_fit_spa)$dispersion
rep$phi
...
#myInfl$calc()
attr(gam_fit_spa, "family")
attributes(gam_fit_spa)


# Diagnostics
obs_fit_df = data.frame(obs = sampData$y_i, glm = fitted.values(gam_fit_spa), CPUEspatial = rep$mu)
...
glm_coefs = coefficients(gam_fit_spa)
...
## plot spline effects
sm <- evaluate_smooth(gam_fit_spa, "s(spline_var)")
