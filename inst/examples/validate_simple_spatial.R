...
opt = nlminb(simple_spatial_model$obj$par, simple_spatial_model$obj$fn, simple_spatial_model$obj$gr, control = list(eval.max = 10000, iter.max = 10000))
rep = simple_spatial_model$obj$report()
sd_rep = sdreport(simple_spatial_model$obj)

glm_fit = glm(y_i ~ factor(year) + factor(fleet_ndx) + factor(region), data = sampData, family = Gamma(link = log))
