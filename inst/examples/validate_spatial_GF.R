...
omega_sigma2 <- 0.1  # marginal variance of the eta field
...
shape = 400 ### for gamma response variable equivalnet to CV = 0.05
1/sqrt(shape)
...
spatial_model_w_omega_NN = configure_obj(observed_df = data, projection_df = full_proj_df, mesh = mesh, family = 2, link = 0, include_omega = T, include_epsilon = F, 
                                      response_variable_label = "y_i", time_variable_label = "year", catchability_covariates = "fleet_ndx",
                                      spatial_covariates = c("region"), spline_catchability_covariates = NULL,
...
rep_omega = spatial_model_w_omega$obj$report(spatial_model_w_omega$obj$env$last.par.best)
rep_omega_NN = spatial_model_w_omega_NN$obj$report(spatial_model_w_omega_NN$obj$env$last.par.best)
sd_rep = sdreport(spatial_model_w_omega$obj)
sd_rep_NN = sdreport(spatial_model_w_omega_NN$obj)



glm_fit_spa = glm(y_i ~ factor(year) + (fleet_ndx) + (region), offset = log(area), data = sampData, family = Gamma(link = log))
